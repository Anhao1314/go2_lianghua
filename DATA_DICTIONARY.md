...
`duration_seconds` 当前为同一 task/seed 快照的最大时间减最小时间，包含可能的暂停与观测缺口，不等同于实际计算时长。人工标签文件位于 data/datasets/manual_labels.csv；键必须唯一，success_rate 在 [0,1]，verdict 为 pass/fail/unknown 或缺失。最终验收因子仅可用于事后分析。
